...
<form class="flex" action="/admin/update-status" method="POST">
    @csrf
    <input class="w-1/12 block px-4 overflow-hidden py-1 text-sm border-[1px] border-solid border-[#ccc]" value="{{ $user->id }}" name="idUser"/>
    ...
    <div class="flex-1 flex px-4 overflow-hidden py-1 text-sm border-[1px] border-solid border-[#ccc]">
        @if ($user->status == '1')
            <input type="submit" value="Inactive" name="inactive"
                class="border-solid border-[1px] border-red-500 text-red-500 cursor-pointer px-2 py-1">
        @else
            <input type="submit" value="Active" name="active"
                class="border-solid border-[1px] border-green-500 text-green-500 cursor-pointer" px-2 py-1>
        @endif
    </div>
</form>
